...
3. **格式严格**：
   ...
   - 不要把一段话拆成很多个用bullet point组成的短句子列表，而要尽可能用完整的段落来组织老师的讲解。只有在讲话人的逻辑本身就符合并列枚举逻辑时，才应该用bullet points组织。
...
同样，以下是一个关于"概念设计"的片段，展示了有序/无序列表的滥用问题：

【❌ 错误的风格】

#### 概念设计（Conceptual Design）
1. 关注现实世界中人对事物的认知和描述。
2. 将现实世界中的实体、属性和它们之间的关系抽象出来，形成概念模型。
3. 例如，在一个教室场景中，将"教室"、"桌椅"、"同学"等抽象为概念层面的描述。
4. 实体关系（ER）模型是概念设计阶段最常用的工具。

【✅ 正确的风格】

#### 概念设计（Conceptual Design）

概念设计阶段的核心任务是将现实世界中人对事物的认知和描述抽象化，提炼出实体、属性及其相互关系，形成独立于任何具体数据库系统的概念模型。以教室场景为例，"教室"、"桌椅"、"同学"等现实对象在这一阶段被抽象为概念层面的描述，而非直接映射为数据库表结构。**实体关系模型（ER 模型）**是完成这一抽象过程最常用的工具，它以直观的图形化方式表达概念层面的设计结果。

核心区别在于：错误的风格将一段完整的知识拆解成大量零碎的短句 bullet point，读起来像提纲而非能让人看懂的笔记，缺乏逻辑连贯性和上下文衔接；
正确的风格用完整段落讲清楚一件事的来龙去脉，bullet point仅在真正需要并列枚举时符合逻辑地使用，同样，有序列表也只在真正包含顺序关系（例如步骤）时使用。
